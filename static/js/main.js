...
function initSocket(){
  try {
    ...
    socket.on('connect', () => {
        const folder = APP_CONFIG.current_folder;
        if (folder) {
...
function removeFileCard(rel){
  ...
  if(el) {
    ...
    function initWebRTC(socket) {
        const folder = APP_CONFIG.current_folder;
        if (!folder || !socket) return;

        webRTCManager = new WebRTCManager(socket, folder);
        webRTCManager.init();

        // Handle incoming files from another peer
        webRTCManager.onFileReceived = (file) => {
            showToast(`Receiving "${file.name}" via WebRTC...`, 'info');
            const id = `webrtc-upload-${Date.now()}`;
            uploadSingleFile({ file: file, id: id });
        };

        // Handle finding a peer to send to
        webRTCManager.onPeerFound = (peerId) => {
            if (window.pendingFileToSend) {
                showToast(`Peer found. Sending ${window.pendingFileToSend.name}...`, 'success');
                webRTCManager.sendFile(window.pendingFileToSend, peerId);
                window.pendingFileToSend = null; // Clear after sending
            }
        };
    }

    // Listen for files shared from the OS via the Service Worker
    navigator.serviceWorker.addEventListener('message', event => {
        if (event.data && event.data.type === 'share-file-ready') {
            const { file } = event.data;
            if (file && webRTCManager) {
                showToast(`File "${file.name}" is ready to send. Finding a peer...`, 'info');
                // Store the file and look for a peer to send it to.
                window.pendingFileToSend = file;
                // For simplicity, we'll just try to find any peer.
                // A more advanced implementation could show a list of peers.
                webRTCManager.findPeer();
            }
        }
    });
