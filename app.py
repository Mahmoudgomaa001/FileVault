...
def api_accounts_create():
    ...
    return jsonify({"ok": True, "folder": name, "switched": make_default, "browse_url": f"/b/{name}"}), 201
...
def api_accounts_switch():
    ...
    return jsonify({"ok": True, "folder": folder, "browse_url": f"/b/{folder}"})
...
def enforce_access_or_redirect(folder: str):
    if not has_folder_access(folder):
        abort(403)
...
def scan_admin(token: str):
    ...
    resp = make_response(redirect(f"/b/{folder}"))
    resp.set_cookie(DEVICE_COOKIE_NAME, device_id, max_age=60*60*24*730, samesite="Lax")
    return resp

@app.route("/api/login_init")
def api_login_init():
    if is_authed():
        return jsonify({"ok": True, "authed": True, "folder": session.get("folder")})

    token = str(uuid.uuid4())
    pending_sessions[token] = {"authenticated": False}
...
@app.route("/api/login_with_default", methods=["POST"])
def api_login_with_default():
    device_id, folder = get_or_create_device_folder(request)
    icon = get_user_icon(folder)
    session["authed"] = True
    session["folder"] = folder
    session["icon"] = icon
    resp = make_response(jsonify({"ok": True, "folder": folder}))
    resp.set_cookie(DEVICE_COOKIE_NAME, device_id, max_age=60*60*24*730, samesite="Lax")
    return resp

@app.route("/api/unlock/<folder>", methods=["POST"])
def api_unlock(folder: str):
    data = request.get_json(silent=True) or {}
    pwd = data.get("password", "")
    if verify_password(folder, pwd):
        access_ok = session.get("access_ok", {})
        if not isinstance(access_ok, dict): access_ok = {}
        access_ok[folder] = True
        session["access_ok"] = access_ok
        return jsonify({"ok": True})
    else:
        return jsonify({"ok": False, "error": "Invalid password"}), 401

@app.route("/api/share_info")
def api_share_info():
    device_id, folder = get_or_create_device_folder(request)
    if not folder:
        return jsonify({"ok": False, "error": "Cannot determine user folder."}), 401
    ...
    if pending_dir.exists():
        for p in sorted(list(pending_dir.iterdir()), key=os.path.getmtime, reverse=True):
            if p.is_file():
                try:
                    _uuid_part, name_part = p.name.split("__", 1)
...
def pc_login(token):
    ...
    next_url = token_data.get("next_url") or "/"
...
def api_browse():
    ...
    if not has_folder_access(folder_name):
    ...
    stats = get_stats(ROOT_DIR / session.get("folder", ""))
...
def handle_404(e):
    if request.path.startswith(("/api/", "/static/", "/socket.io/")):
        # For API, static files, and socket.io, return a standard JSON 404
        return jsonify(error="Not Found"), 404
    # For any other path, serve the SPA shell
    return send_from_directory('static', 'index.html')
